...
# 检查构建结果
if [ -f "build/distributions/bigclean-scanner-1.0.2.zip" ]; then
    echo "✓ 插件构建成功!"
    echo "插件包位置: build/distributions/bigclean-scanner-1.0.2.zip"
    
    # 显示文件信息
    file_size=$(du -h "build/distributions/bigclean-scanner-1.0.2.zip" | cut -f1)
    echo "插件包大小: $file_size"
    
    # 创建发布目录
    mkdir -p release
    cp "build/distributions/bigclean-scanner-1.0.2.zip" "release/"
    echo "✓ 插件包已复制到 release/ 目录"
    
    # 显示插件包内容
    echo ""
    echo "=== 插件包内容 ==="
    unzip -l "build/distributions/bigclean-scanner-1.0.2.zip"
    ...
    echo "4. 选择 release/bigclean-scanner-1.0.2.zip"
